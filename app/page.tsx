...
export default function Page() {
    ...
    useEffect(() => {
      async function loadImpact() {
        ...
        try {
          ...
          console.error(`Failed to load impact for operator ${operator.id}:`, error);
        } finally {
          setIsLoadingImpact(false);
        }
      }
  ...
  const handleCampaignAssess = (campaign: Campaign) => {
  ...
  if (isLoading) {
    return (
      <div className="app-root h-full">
        <div className="app-overlay h-full flex items-center justify-center">
          <div className="text-center">
            <div className="text-white text-xl mb-4">Loading NdaY'Fako Platform...</div>
            <div className="text-white/50 text-sm">Connecting to Firestore...</div>
          </div>
        </div>
      </div>
    );
        ...
        <div className="app-root h-full">
          <div className="app-overlay h-full flex flex-col">
            ...
            <div className="fixed top-0 left-0 w-full z-50 bg-black/20 backdrop-blur-md border-b border-white/10">
              ...
              <div className="text-xs text-white/50 px-4 py-1 bg-black/30">
                Debug: {campaigns.length} campaigns loaded
              </div>

              {campaigns.length === 0 ? (
                <div className="px-6 py-3 text-white/50">
                  No campaigns available. Check Firestore connection.
                </div>
              ) : (
                <CampaignTicker
                  campaigns={campaigns}
